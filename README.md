...
* **Video Information Extraction** - Retrieve detailed metadata such as title, description, duration, views, and more
* **Caption Extraction** - Automatically extract and parse video captions/subtitles with timestamps
* **AI-Powered Summarization** - Generate comprehensive summaries using local **LM Studio** models (no expensive cloud APIs)
* **Key Points Extraction** - Identify and extract 5-8 important concepts and takeaways from the video
* **Timestamped Analysis** - Segment-wise insights divided into 5 parts with accurate timestamps
...
* **Cost-Effective** - Uses local LM Studio models instead of paid cloud APIs
* **Efficient** - Optimized API usage without sacrificing analysis quality
...
```bash
chmod +x start-tmux.sh start-backend.sh start-frontend.sh
./start-tmux.sh
```
...
├── start-tmux.sh                   # Combined startup script
├── README.md                       # This file
...
3. **Click "Analyze Video"**
   - Processing time typically 20-40 seconds depending on video length
...
Note: Cloud deployments require cloud-based LM models instead of local LM Studio.

For production, consider:
- AWS Lambda with API Gateway
- Google Cloud Run
- Azure Functions
- Using OpenAI/Claude APIs instead of LM Studio
...
1. **Optimize LM Studio Model**
   - Use smaller models like `smollm` for faster processing
